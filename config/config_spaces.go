...
func (c *DigitalOceanSpacesConfig) RegisterFlags(f *flag.FlagSet) {
	flag.StringVar(&c.Region, "spaces.region", getenv("SPACES_REGION", "sfo3"), "Region to use in querys")
	flag.StringVar(&c.Endpoint, "spaces.endpoint", getenv("SPACES_ENDPOINT", "digitaloceanspaces.com"), "Endpoint to use for queries")
}
