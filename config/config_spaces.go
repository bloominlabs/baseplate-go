...
func (c *DigitalOceanSpacesConfig) RegisterFlags(f *flag.FlagSet) {
	flag.StringVar(&c.Region, "spaces.region", GetEnvDefault("SPACES_REGION", "sfo3"), "Region to use in querys")
	flag.StringVar(&c.Endpoint, "spaces.endpoint", GetEnvDefault("SPACES_ENDPOINT", "digitaloceanspaces.com"), "Endpoint to use for queries")
	flag.StringVar(&c.AccessKeyID, "spaces.access_key_id", GetEnvDefault("AWS_ACCESS_KEY_ID", GetEnvDefault("SPACES_ACCESS_KEY_ID", "")), "Spaces Access Key ID for authentication")
	flag.StringVar(&c.SecretKey, "spaces.secret_key", GetEnvDefault("AWS_SECRET_ACCESS_KEY", GetEnvDefault("SPACES_SECRET_ACCESS_KEY", "")), "Spaces Secret Access Key for authentication")
}
