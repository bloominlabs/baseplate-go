...
func (c *NomadConfig) RegisterFlags(f *flag.FlagSet) {
	f.StringVar(&c.Address, "nomad.addr", GetEnvStrDefault("NOMAD_ADDR", "localhost:4646"), "hostname:port to connect to the nomad server")
	f.StringVar(&c.Token, "noamd.token", GetEnvStrDefault("NOMAD_TOKEN", ""), "Token to use to authenticate to nomad")
}
